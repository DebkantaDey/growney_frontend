...
export default function Ecosystem() {
    ...
    useEffect(() => {
        axios.get(`https://rankterminal.com/growney/public/index.php/api/eco-system?nolimit=1`)
            .then((response) => {
                ...
                <table className="table-auto w-full overflow-x-auto">
                    <thead style={{ backgroundColor: '#facb92', color: 'white' }} className='relative'>
                        <tr className="size">
                            ...
                            <th scope="col" style={{ backgroundColor: '#facb92', minWidth: '190px' }} className='sticky left-0 bg-orange-400 pl-6'>Name</th>
                            <th>Socials</th>
                            <th scope="col" className='whitespace-nowrap pl-6' style={{ minWidth: '200px' }}>Project</th>
                        </tr>
                    </thead>
                    {data.length !== 0 ?
                        <tbody>
                            {data?.map(item =>
                                <tr className="item-name size border-b border-slate-400 dark:text-white" key={item?.id}>
                                    <td className="name sticky left-0 items-center px-6 dark:bg-gray-900">
                                        <Watchlist tableId={item.id} tableName={"eco_system"} />
                                    ...
                                    <td className='whitespace-nowrap px-6' style={{ minWidth: '200px' }}>{item?.project}</td>
                                </tr>
                            )}
                        </tbody>
                        :
